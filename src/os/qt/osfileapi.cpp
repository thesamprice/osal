...
int32 OS_FS_Init(void)
{
    ...
    if(OS_FDTableMutex != 0x0)
        delete OS_FDTableMutex;
    OS_FDTableMutex = new QMutex();
...
int32 OS_rename (const char *old, const char *new_name)
{
    int status,i;
    ...
    */
    ...
    if (strlen(new_name) >= OS_MAX_PATH_LEN)
        return OS_FS_ERR_PATH_TOO_LONG;
    ...
    */
    ...
    if (OS_check_name_length(new_name) != OS_FS_SUCCESS)
        return OS_FS_ERR_NAME_TOO_LONG;
    ...
    ** Translate the path
    */
    if ( OS_TranslatePath(new_name, (char *)new_path) != OS_FS_SUCCESS )
    {
        return OS_FS_ERR_PATH_INVALID;
    }
    ...
    if (status == 0)
    {
        for ( i =0; i < OS_MAX_NUM_OPEN_FILES; i++) 
        {
            ...
            {
                strncpy (OS_FDTable[i].Path, new_name, OS_MAX_PATH_LEN);
            } 
        }
        return OS_FS_SUCCESS;
    }
